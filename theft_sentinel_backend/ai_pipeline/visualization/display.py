...
Source: Updated_AI_Engine_v2 (best visualization with theft overlays) + minor
...
class Visualizer:
    ...
    def draw_detections(
        ...
        detections: list,
        camera_id: int,
        theft_scores: dict = None,   # legacy kwarg — ignored; scores are in det dicts
    ) -> np.ndarray:
        """
        ...
        The 'theft_scores' keyword arg is accepted for backward-compat
        with old callers (inference_runner / continuous_monitor) but is
        not used — all theft state comes from the detection dicts.
        """
        ...
        for det in detections:
            ...
            theft_score = det.get("theft_score", det.get("x3d_score", 0.0))
            is_susp     = det.get("is_suspicious", consecutive > 0)

            # Box color: theft > suspicious > global-ID color
            if is_theft or in_cooldown:
                box_color = COLOR_THEFT
            elif is_susp or consecutive > 0:
                box_color = COLOR_SUSPICIOUS
            elif global_id and global_id > 0:
                box_color = get_color(int(global_id))
            else:
            ...
            # Build label text
            label_parts = []
            if Config.VIS_SHOW_GLOBAL_ID and global_id and global_id > 0:
