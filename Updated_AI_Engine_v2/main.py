...
from theft_detection.x3d_detector import GlobalTheftDetector


# ── IoU helper (Updated_AI_Engine: used for YOLO-bbox alignment) ──────────────
...
class TheftSentinelMCMT:
    ...
    # ── Global match helper (New_MCMT dual-strategy) ───────────────────────────

    def _match_globally(self, embedding: np.ndarray, camera_id: int) -> tuple:
        """
        Dual-strategy global identity matching (from New_MCMT).
    ...
    def process_camera(self, camera_id: int, frame_bgr: np.ndarray) -> tuple:
        ...
        Incorporates three critical fixes from Updated_AI_Engine:
        ...
        for track_info, embedding in zip(track_list, embeddings):
            ...
            if existing_gid is not None:
                ...
                # New track — dual-strategy global matching (New_MCMT)
    ...
    def run(self):
        ...
        Frame pacing uses EMA FPS (from New_MCMT) — smooth readout instead of
        jumpy per-second resets.
        Theft alerts use rising-edge counting (from Updated_AI_Engine) — one
        console alert per event, not one per frame.
        """
        ...
        try:
            while self._running:
                ...
                # EMA FPS (New_MCMT — smooth, stable readout)
    ...
    # ── Detailed stats (New_MCMT style with per-identity breakdown) ────────────
...
def parse_args():
    """Parse command-line arguments (superset of both original CLIs)."""
    ...
    parser.add_argument("--yolo-model", type=str, default=None,
        help=f"YOLO model variant (default: {Config.YOLO_MODEL})")
