...
Source: New_MCMT (best multi-camera handling logic).
...
class CameraStream:
    ...
    Network streams use grab()/retrieve() to drain the internal buffer and
    always deliver the freshest frame. File sources rate-limit to TARGET_FPS.
    """
    ...
    def get_frame(self) -> tuple:
        ...
        Returns:
            (success: bool, frame: np.ndarray | None) — frame is BGR.
        """
        with self._lock:
...
class MultiCameraManager:
    ...
    and retrieve the latest frame from any or all cameras.
    """
    ...
    def add_camera(self, source, camera_id: int) -> bool:
        ...
        Args:
            source: Video source (file path, RTSP/HTTP URL, or device index).
    ...
    def get_frame(self, camera_id: int) -> tuple:
    ...
    def get_all_frames(self) -> dict:
        ...
        Returns:
            Dict mapping camera_id → BGR frame (only cameras with valid frames).
        """
        frames = {}
