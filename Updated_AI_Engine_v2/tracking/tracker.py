...
Source: New_MCMT / Updated_AI_Engine (identical logic; New_MCMT version kept).
...
depending on the library version.  All callers in main.py normalise them
with str() before using them as dict keys.
"""
    ...
    def update(self, detections: list, frame: np.ndarray) -> list:
        ...
        Returns:
            List of confirmed tracks, each a dict with:
                - 'track_id': local per-camera ID (str after normalisation in main.py)
                - 'bbox':     [x1, y1, x2, y2]
                - 'confirmed': bool
        """
        if not detections:
